...
public static class Simplex {
	...
	///	Created: March 18th, 2019 by Thomas Carrella
	///	Based on WardBenjamin's SimplexNoise GitHub Project
	...
	public static float[,] Calc2D (int width, int height, int seed, NoiseSettings noiseSettings, Vector2 offset) {
		Seed = seed;
		...
		for (var i = 0; i < width; i++) {
			for (var j = 0; j < height; j++) {
				...
				for (int l = 0; l < noiseSettings.octaves; l++) {
					...
					currentSample += Generate(sampleX, sampleY) * noiseSettings.amplitudeTable[l];
				}
				values[i, j] = currentSample;
			}
		}
		return values;
	}
